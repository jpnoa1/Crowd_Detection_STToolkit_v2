...
//Packet filtering
//#define PACKET_POWER_FILTRATION 0
...
char **mobile_manufacturers_list;

//Different data packets MAC addresses list 
#define DIFF_MAC_ADDRS_SIZE 50	//Maximum number of different MAC addresses
...
static const char * OUI_PATHS[]
	= {"/home/kali/Desktop/oui-list.txt",
	   "./airodump-ng-oui.txt",	
						   ...
						   sqlite3 *db)
{
	...
	//Different data packets MAC addresses
	if(seconds == 0 && stop_process == 0){
		stop_process = 1;

		//Store each captured MAC address in local database
		for(int c = 0; c < pos_different_mac_addresses; c++){

			//Anonymize MAC address

			uint64_t u64t_mac_dp;
			int seed = 11;
			char footprint_mac_dp[17];

			u64t_mac_dp = t1ha0(different_mac_addresses[c], sizeof(different_mac_addresses[c]), (uint64_t)seed);
			snprintf(footprint_mac_dp, sizeof(footprint_mac_dp), "%llX", u64t_mac_dp);

			putInToDataPacketsDB(footprint_mac_dp, different_mac_addresses_manuf[c], db);
		}

		//Empty different data packets MAC address list
		pos_different_mac_addresses = 0;
	}else if(seconds != 0){
		stop_process = 0;
	}
	...
	switch (h80211[1] & IEEE80211_FC1_DIR_MASK)
	{
		...
		case IEEE80211_FC1_DIR_FROMDS: /* To_DS = 0  From_DS = 1 */
		case IEEE80211_FC1_DIR_DSTODS: /* To_DS = 1  From_DS = 1 */
	...
	char bssid_addr_ap[50];
		sprintf(bssid_addr_ap , "%02X:%02X:%02X:%02X:%02X:%02X",
					 bssid[0],
					 bssid[1],
					 bssid[2],
					 bssid[3],
	...
	char bssid_addr[50];
		sprintf(bssid_addr , "%02X:%02X:%02X:%02X:%02X:%02X",
					 bssid[0],
					 bssid[1],
					 bssid[2],
					 bssid[3],
	...
	power=ri->ri_power;
	int best_power ;
	best_power=st_cur->best_power;
	int channel;
	channel=st_cur->channel;
	char * manuf ;
	manuf =st_cur->manuf; /* the client's manufacturer */
...
skip_station:
	...
	char received_power[10];
	sprintf(received_power, "%d", power);
	strcat(received_power, " dB");
	...
	if (h80211[0] == IEEE80211_FC0_SUBTYPE_PROBE_REQ && st_cur != NULL)  
	{
		...
		#ifdef PACKET_POWER_FILTRATION
		if (power > PACKET_POWER_FILTRATION){
		#endif

			// Check real or virtual MAC address

			bool is_mac_addr_random;

			if((h80211[10] & 0x02) == 0){
				is_mac_addr_random = false;
			}else{
				is_mac_addr_random = true;
			}

			if(is_mac_addr_random == false){

				// Check if MAC address belongs to a mobile manufacturer

				for( int s = 0; s < total_lines; s++){

					if(strstr(manuf, mobile_manufacturers_list[s]) != NULL){

						// Store anonymized MAC address in local database

						uint64_t u64t_mac;
						int seed = 11;
						char footprint_mac[17];

						u64t_mac = t1ha0(mac_address, sizeof(mac_address), (uint64_t)seed);
						snprintf(footprint_mac, sizeof(footprint_mac), "%llX", u64t_mac);

						//Para debug
						//printf("Probe Request | Source: %s | Footprint: %s | SEQ: %i | Power: %s | Manuf: %s \n", mac_address, footprint_mac, seq, received_power, manuf);

						putInToProbeRequestsDB(footprint_mac, manuf, db);
				...
				int ssid_length;
				int supp_rates_length;
				int extended_supp_rates_length;
				int DS_param_length;
				int HT_capabilities_length;
				int extended_cap_length;
				int VHT_capabilities_length;
				int RM_enabled_cap_length;
				int interworking_length;
				int supp_operating_classes_length;
				int vendor_specific_length;

				unsigned char array_v[440] = {};
				int pos = 0;
				int seed = 3;
				...
				while (p < h80211 + caplen)
				{
					if (p + 2 + p[1] > h80211 + caplen)break; 
						...
						if(p[0] == 0x032){
							...
							extended_supp_rates_length = p[1];
							...
							for(i = 0; i < extended_supp_rates_length; i++)
							{
						...
						if(p[0] == 0x2D){
							...
							for(i = 0; i < HT_capabilities_length; i++)
							{
						...
						if(p[0] == 0x7F){
							...
							extended_cap_length = p[1];

							array_v[pos] = p[0];
							pos++;

							
							for(i = 0; i < extended_cap_length; i++)
							{
								array_v[pos] = p[2 + i];
								pos++;
						...
						if(p[0] == 0xBF){
							...
							for(i = 0; i < VHT_capabilities_length; i++)
							{
						...
						if(p[0] == 0x46){
							...
							for(i = 0; i < RM_enabled_cap_length; i++)
							{
						...
						if(p[0] == 0x6B){
							...
							for(i = 0; i < interworking_length; i++)
							{
						...
						if(p[0] == 0x3B){
							...
							supp_operating_classes_length = p[1];
							...
							for(i = 0; i < supp_operating_classes_length; i++)
							{
						...
						if(p[0] == 0xDD){
							...
							for(i = 0; i < vendor_specific_length; i++){
				...
				unsigned char new_array[pos];

				for(int d = 0; d < pos; d++)
				{
					new_array[d]=array_v[d];
				}
				
				u64t = t1ha0(new_array, sizeof(new_array), (uint64_t)seed);
				snprintf(footprint, sizeof(footprint), "%llX", u64t);

				//Para debug
				//printf("Probe Request | Source: %s | Footprint: %s | SEQ: %i | Power: %s | Manuf: %s \n", mac_address, footprint, seq, received_power, manuf);

				putInToProbeRequestsDB(footprint, manuf, db);
...
skip_probe:
	...
	if ((h80211[0] & IEEE80211_FC0_TYPE_MASK) == IEEE80211_FC0_TYPE_DATA)
	{
		...
		if (power > PACKET_POWER_FILTRATION){
			...
			//Search different MAC address
			int same_mac_addr_found = 0;

			for(int i=0; i < pos_different_mac_addresses; i++){

				if(strcmp(different_mac_addresses[i],mac_address) == 0){
					same_mac_addr_found = 1;

					//Update MAC address power
					strcpy(different_mac_addresses_power[i], received_power);
				}
			}
			
			//Insert MAC address in list (Reject broadcast MAC addresses)
			if(same_mac_addr_found == 0 && pos_different_mac_addresses < number_different_mac_addresses && strcmp(mac_address, "FF:FF:FF:FF:FF:FF") != 0){

				char first[3];
				memcpy(first,mac_address,2);
				first[2] = '\0';
				int first_num = (int)strtol(first, NULL, 16);


				//Reject multicast MAC addresses (1st byte less significant bit to '1')
				if ( (first_num & 0x01) != 1 ){

					strcpy(different_mac_addresses[pos_different_mac_addresses], mac_address);
					strcpy(different_mac_addresses_power[pos_different_mac_addresses], received_power);
					strcpy(different_mac_addresses_manuf[pos_different_mac_addresses], manuf);

					pos_different_mac_addresses++;
				}
...
/* Extract a name:value pair from a null-terminated line of JSON.value
 *
...
static void sighandler(int signum)
{
	...
	if (signum == SIGINT || signum == SIGTERM)
	{
		...
		for (size_t i = 0; i < total_lines; i++)
			free(mobile_manufacturers_list[i]);
	
		free(mobile_manufacturers_list);
...
int main(int argc, char * argv[])
{
	...
	*/
	...
	//Mobile device manufacturers list load - text
	FILE *fptr = NULL;

	fptr = fopen("/home/kali/Desktop/Mobile_device_manufacturers.txt", "r");	//IMPORTANT NOTE: Text file with UTF-8 codification!!

	if( fptr == NULL){
		printf("Error on opening file - Mobile_device_manufacturers.txt.\n");
		return 1;
	}
	
	mobile_manufacturers_list = malloc(sizeof(char *) * 1024);
	...
	do 
	{
		...
		if (feof(fptr))
		{
			...
			if (total_chars != 0)
			{        
				mobile_manufacturers_list[total_lines] = realloc(mobile_manufacturers_list[total_lines], total_chars + 1 );
				
				mobile_manufacturers_list[total_lines][total_chars] = '\0';
		...
		if (total_chars == 0) mobile_manufacturers_list[total_lines] = malloc(1024); 

		if (c != '\n')
		{

			mobile_manufacturers_list[total_lines][total_chars] = c; 
			...
			mobile_manufacturers_list[total_lines][total_chars - 1] = '\0';
	...
	mobile_manufacturers_list = realloc(mobile_manufacturers_list, sizeof(char *) * total_lines);
    ...
    char *err_msg = 0;
	#ifdef HAVE_SQLITE
		int rc = sqlite3_open("/home/kali/Desktop/DB/DeviceRecords.db", &db);
		...
		if (opt.s_file == NULL && lopt.s_iface != NULL)
		{
			for (i = 0; i < lopt.num_cards; i++)
			{
				if (FD_ISSET(fd_raw[i], &rfds)) // NOLINT(hicpp-signed-bitwise)
				{
					...
					wi_read_failed = 0;
					dump_add_packet(h80211, caplen, &ri, i, db);
				}
			}
		}
